...
impl Debugger {
    pub fn new() -> Self {
        ...
        for &op in &[
            0x04, 0x44, 0x64, 0x0C, 0x14, 0x34, 0x54, 0x74, 0xD4, 0xF4, 0x1A, 0x3A, 0x5A, 0x7A, 0xDA, 0xFA,
            0x80, 0x1C, 0x3C, 0x5C, 0x7C, 0xDC, 0xFC,
        ] {
    ...
    pub fn on_instruction(&mut self, cpu: &Cpu6502, bus: &Bus) {
        ...
        if self.trace_enabled {
            ...
            let line = format!(
                ...
                pc, opcode, operand1, operand2, name, cpu.a, cpu.x, cpu.y, cpu.status, cpu.stkp
            );
    ...
    pub fn detect_stuck(&self, cpu: &Cpu6502, bus: &Bus) -> Option<String> {
        ...
        if matches!(op, 0x10 | 0x30 | 0x50 | 0x70 | 0x90 | 0xB0 | 0xD0 | 0xF0) {
            ...
            if target == pc {
                return Some(format!(
                    "CPU stuck: branch to self at ${:04X} ({})",
                    pc, self.opcode_names[op as usize]
                ));
            }
        }
