...
impl Mapper for Mmc2 {
    ...
    fn cpu_write(&mut self, addr: u16, val: u8, data: &mut CartData) -> bool {
        match addr {
            ...
            0xB000..=0xBFFF => {
                self.chr_banks[0] = val & 0x1F;
                true
            }
            0xC000..=0xCFFF => {
                self.chr_banks[1] = val & 0x1F;
                true
            }
            0xD000..=0xDFFF => {
                self.chr_banks[2] = val & 0x1F;
                true
            }
            0xE000..=0xEFFF => {
                self.chr_banks[3] = val & 0x1F;
                true
            }
            0xF000..=0xFFFF => {
    ...
    fn ppu_read(&mut self, addr: u16, data: &CartData) -> Option<u8> {
        if addr <= 0x1FFF {
            let bank = if addr < 0x1000 {
                ...
                if self.latch[1] == 0xFD { self.chr_banks[2] } else { self.chr_banks[3] }
            };
            let offset = bank as usize * 0x1000 + (addr & 0x0FFF) as usize;
            let result = if offset < data.chr.len() { Some(data.chr[offset]) } else { Some(0) };
